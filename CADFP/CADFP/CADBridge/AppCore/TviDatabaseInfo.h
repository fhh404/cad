...
public:
    ...
    TviDatabaseInfo() : m_iLoadTime(0), m_iVectorizingTime(0), m_iTvCreationTime(0), m_iFirstUpdateTime(0)
        , m_iCdaCreationTime(0), m_type(kFromFile) {}
    ~TviDatabaseInfo() {}
    ...
    // returns the cda creation time (in milliseconds)
    ...
    // set the time of cda creation (in milliseconds)
    virtual void setCDATreeCreationTime(OdInt64 time) { m_iCdaCreationTime = time; }
