...
#define OD_TV_APP_NAME OD_T("CADViewer")
...
OdTvLayersIteratorPtr TviCore::getLayers()
{
    OdTvDatabasePtr dbPtr = m_TvDatabaseId.openObject(OdTv::kForWrite);
    OdTvResult rc;
    OdTvLayersIteratorPtr layersIter = dbPtr->getLayersIterator(&rc);

    return layersIter;
    
//    OdGiTextStyleTraits style ;
//    style->get
//    OdTvDatabasePtr pDbP = m_TvDatabaseId.openObject(OdTv::kForWrite);
//    OdTvDatabase pDb =  pDbP.get();
//
//    OdTvDbHostAppServices pHostApp =  dbPtr->;
//    pHostApp->f
//
...
void TviCore::searchDBDataContent()
{
    ...
    OdTvResult rc;
    OdTvDatabasePtr dbPtr = m_TvDatabaseId.openObject(OdTv::kForRead);
    OdTvBlocksIteratorPtr BIPtr =  dbPtr->getBlocksIterator(&rc);
    
    for (; !BIPtr->done(); BIPtr->step()) {
    
        OdTvBlockId blockId = BIPtr->getBlock(&rc);
        
        OdTvBlockPtr blockPtr = blockId.openObject(OdTv::kForRead);
       
 
        OdTvEntitiesIteratorPtr entityPtr = blockPtr->getEntitiesIterator();
...
//            NSLog(@"==%@",OdString2NSString(entityPtr->get()));
            ...
            OdTvGeometryDataIteratorPtr geometryDataIterator =  entityPtr->getGeometryDataIterator();
            
            for (; !geometryDataIterator->done(); geometryDataIterator->step()) {
                OdTvGeometryDataId textId = geometryDataIterator->getGeometryData();
...
OdUInt32 TviCore::getIndexedColor(int colorIndex)
{
//    OdTvGsDevicePtr pDev = m_TvDeviceId.openObject(OdTv::kForWrite);
//
//    int numColors = 256;
//    OdUInt32 *palette = new OdUInt32[numColors];
//    for (int i = 0; i < numColors; i++)
//    {
//        palette[i] = ODRGB(i, i, i); // from black to white
//    }
//
//    pDev->setLogicalPalette(palette, numColors);
//
//    const OdUInt32* pPalette = pDev->getLogicalPalette(numColors);
//
//    OdUInt32 color0 = pPalette[colorIndex]; // get first color, for example
//
//    return color0;
    
    OdTvColorDef color = OdTvColorDef(colorIndex);
    //OdTvColorDef color
    if (OdTvColorDef::kIndexed == color.getType())
    {
      OdTvGsDevicePtr pDevice = m_TvDeviceId.openObject();
      if (!pDevice.isNull())
      {
        int numColors = 256;
        const OdUInt32* pPalette = pDevice->getLogicalPalette(numColors);
        OdUInt32 colorRGB = pPalette[color.getIndexedColor()];
...
void TviCore::changeBackground(int blackOrWhite)
{
  
    OdTvResult rc;
    
    OdTvGsDevicePtr pDevice0 = m_TvDeviceId.openObject(OdTv::kForWrite, &rc);
 
    if (blackOrWhite) {
        pDevice0->setBackgroundColor(ODRGB(255,255,255));
    }else
    {
        pDevice0->setBackgroundColor(ODRGB(0,0,0));
    }
   
 
}

void TviCore::setSettingLinesColor(TviCore::DrawLinesColor linesColorIndex)
{
    switch (linesColorIndex) {
        case Lines_White:
            linesColor = OdTvColorDef(255, 255, 255);
            break;
        case Lines_Red:
            linesColor = OdTvColorDef(251, 0, 7);
            break;
        case Lines_Green:
            linesColor = OdTvColorDef(140, 235, 19);
            break;
        case Lines_Orange:
            linesColor = OdTvColorDef(252, 65, 5);
            break;
        case Lines_Black:
            linesColor = OdTvColorDef(0, 0, 0);
            break;
        case Lines_Pink:
            linesColor = OdTvColorDef(233, 0, 133);
            break;
        case Lines_yellow:
            linesColor = OdTvColorDef(252, 132, 7);
            break;
        case Lines_Brown:
            linesColor = OdTvColorDef(75, 24, 1);
            break;
...
void TviCore::cleanLastMarks()
{
 
    OdTvDatabasePtr pDatabase = m_TvDatabaseId.openObject(OdTv::kForWrite);
    if (linesFlag) {
        NSString *modelName = [NSString stringWithFormat:@"$ODA_TVVIEWER_MARKUPS_%d",linesFlag-1];
        OdString mname = NSString2OdString(modelName);
        OdTvModelId modelId = pDatabase->findModel(mname);

        pDatabase->removeModel(modelId);
...
void TviCore::cleanMarks()
{
 
    TviCore::finishDragger();
    OdTvDatabasePtr pDatabase = m_TvDatabaseId.openObject(OdTv::kForWrite);;
//     pDatabase->clearModels();
 
    
    
    for (int i = 0; i<linesFlag; i++) {
        NSString *modelName = [NSString stringWithFormat:@"$ODA_TVVIEWER_MARKUPS_%d",i];
        OdString mname = NSString2OdString(modelName);
        OdTvModelId modelId = pDatabase->findModel(mname);
...
//        if(m_TvMarkupsModelId.isNull())
        NSString *modelName = [NSString stringWithFormat:@"$ODA_TVVIEWER_MARKUPS_%d",linesFlag];
        linesFlag++;
        OdString mname = NSString2OdString(modelName);
            m_TvMarkupsModelId =  pDatabase->findModel(mname);
        if(m_TvMarkupsModelId.isNull())
            m_TvMarkupsModelId = pDatabase->createModel(mname, OdTvModel::kMain, true);
        
    }
//    else
//    {
//        OdTvDatabasePtr pDatabase = m_TvDatabaseId.openObject(OdTv::kForWrite);
//        pDatabase->removeModel(m_TvMarkupsModelId);
...
void TviCore::disableMarkups() {
    if (m_TvDatabaseId.isNull() || (m_TvMarkupsModelId.isNull() && linesFlag <= 0))
        return;
    ...
    int markupModelCount = linesFlag > 0 ? linesFlag : 1;
    for (int index = 0; index < markupModelCount; ++index)
    {
        OdTvModelId markupModelId;
        if (linesFlag > 0)
        {
            NSString *modelName = [NSString stringWithFormat:@"$ODA_TVVIEWER_MARKUPS_%d", index];
            markupModelId = pDatabase->findModel(NSString2OdString(modelName));
        }
        else
            markupModelId = m_TvMarkupsModelId;

        if (markupModelId.isNull())
            continue;

        OdTvModelPtr pMarkupModel = markupModelId.openObject(OdTv::kForWrite);
        if (pMarkupModel.isNull())
            continue;
...
void TviCore::showMarkups()
{
    ...
    for (int index = 0; index < markupModelCount; ++index)
    {
        ...
        OdTvModelPtr modelPtr = markupModelId.openObject(OdTv::kForWrite);
        if (modelPtr.isNull())
            continue;

        for (OdTvEntitiesIteratorPtr it = modelPtr->getEntitiesIterator(); !it->done(); it->step())
        {
            OdTvEntityPtr entity = it->getEntity().openObject(OdTv::kForWrite);
            if (entity.isNull())
                continue;

            if (entity->getName().compare(OD_TV_MARKUP_TEMP_ENTITY) != 0)
...
bool TviCore::saveMarkup(NSString *markupName)
{
    ...
    if (!pView.isNull())
    {
        ...
        OdTvResult rc;
        rc = pEn->appendUserData(data, m_appTvId);
        OdTvResult rrc;
    }
